...
from fairseq import checkpoint_utils, file_utils, utils
from fairseq.data import data_utils, transforms
...
class Generator(object):
    ...
    @classmethod
    def from_pretrained(cls, model_name_or_path, checkpoint_file='model.pt', data=None, **kwargs):
        model_path = file_utils.load_archive_file(model_name_or_path)
        checkpoint_path = os.path.join(model_path, checkpoint_file)

        # convenience hack for loading data and BPE codes from model archive
        if data is not None:
            kwargs['data'] = file_utils.load_archive_file(data)
        for arg, file in {
            'bpe_codes': 'bpecodes',
        ...
        model, args, task = checkpoint_utils._load_model_ensemble(
            [checkpoint_path],
